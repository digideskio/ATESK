...
namespace TevelATE
{
    ...
    public class SerialPortFriendlyName
    {
        public string GetComPortsFriendlyNames(string name)
        {
            foreach (COMPortInfo comPort in COMPortInfo.GetCOMPortsInfo())
            {
                Console.WriteLine(string.Format("{0} – {1}", comPort.Name, comPort.Description));
                if (comPort.Description.Contains(name) == true)
                {
                    return comPort.Name;
                }
            }
        ...
        public string ComPortNameFromFriendlyNamePrefix(string friendlyNamePrefix)
        {
                ...
                if (h.ToInt32() != INVALID_HANDLE_VALUE)
                {
                    ...
                    while (true)
                    {
                            ...
                            BUFFER_SIZE, out RequiredSize))
                        {
                            ...
                            if (!friendlyName.Contains(friendlyNamePrefix))
                                continue;
